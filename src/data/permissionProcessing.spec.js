...
describe('data/permissionProcessing',()=>{
    describe('isAllowedToUpdateTask()',()=>{
        ...
        it('returns false if task or apiPermission are falsey',()=>{
            ...
            const testArgs = [
                [void 0, testPermissions],
                [null, testPermissions],
                [testTask, void 0],
                [testTask, null],
            ];
            testArgs.forEach(args =>{
                expect(isAllowedToUpdateTask(...args),`Expect ${args} to give false`).to.be.false;
            });
        });
        ...
        describe('With permissions to update OWN tasks',()=>{
            it('returns false if task\'s staffName does not match token\'s staffName.',()=>{
                ...
                expect(isAllowedToUpdateTask(testTask, testPermissions)).to.be.false;
            });
            ...
            it('returns true if task\'s staffName does match token\'s staffName.',()=>{
                ...
                expect(isAllowedToUpdateTask(testTask, testPermissions)).to.be.true;
            });
        ...
        describe('With permissions to update ALL tasks',()=>{
            it('returns true, whether the  task\'s staffName matches or not.',()=>{
                ...
                expect(isAllowedToUpdateTask(testTaskMatched, testPermissions)).to.be.true;
                expect(isAllowedToUpdateTask(testTaskNotMatched, testPermissions)).to.be.true;
            });
        });
    });
});
